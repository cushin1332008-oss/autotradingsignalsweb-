...
if os.path.exists(secret_path):
    ...
    raise RuntimeError("Không tìm thấy tệp Firebase credentials.")
...
def send_telegram_alert(item):
    ...
    try:
        ...
        text = (
            ...
            f"🎯 Bậc tin cậy BTC/Đa khung: <b>{item['confluence_pct']}%</b>\n\n"
            ...
            f"⚡ <b>Đòn bẩy:</b> {item['leverage']}x | <b>Margin:</b> {item['margin_pct']}%\n"
            f"💡 <b>Lý do:</b> {item['reason']}\n"
            f"🕒 <i>{item['time_str']}</i>"
        )
        url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
        session.post(url, json={"chat_id": TELEGRAM_CHAT_ID, "text": text, "parse_mode": "HTML"}, timeout=8)
    except Exception as e:
        print(f"[⚠️ TELEGRAM ERROR] {e}")

def get_klines(symbol: str, interval: str, limit: int = 200):
    try:
        res = session.get("https://api.binance.com/api/v3/klines", params={"symbol": symbol, "interval": interval, "limit": limit}, timeout=8)
        raw = res.json()
...
def scan_and_push_to_firebase():
    print(f"\n[🚀 SCAN] Khởi chạy lúc: {now_vn_str()}")
    watchlist = ["ETHUSDT", "SOLUSDT", "BNBUSDT", "XRPUSDT", "NEARUSDT", "DOGEUSDT", "ADAUSDT"]

    # Step 1: Quét BTC trước tiên để lấy Trend làm chuẩn
    btc_tf_results = analyze_symbol_tf("BTCUSDT")
    signals_to_upload = {}

    # Lấy tín hiệu cho BTC nếu có
    btc_signals = generate_all_signals(btc_tf_results, is_btc=True)
    for profile_key, signal in btc_signals.items():
        sizing = calc_position_sizing(signal["entry_1"], signal["stop_loss"], signal["confluence_pct"], profile_key)
        signals_to_upload[f"BTCUSDT_{profile_key}"] = {
            "symbol": "BTCUSDT", **signal, **sizing, "time_str": now_vn_str()
        }

    # Step 2: Lọc Altcoins theo BTC Context
    ...
    print(f"[🔥 FIREBASE] Đã cập nhật thành công {len(signals_to_upload)} tín hiệu phù hợp Trend BTC.")
...
@app.route("/")
def health_check():
    return {"status": "ok", "time": now_vn_str()}

if __name__ == "__main__":
    threading.Thread(target=lambda: app.run(host="0.0.0.0", port=int(os.environ.get("PORT", 10000))), daemon=True).start()
    scan_and_push_to_firebase()
    scheduler = BackgroundScheduler()
    scheduler.add_job(scan_and_push_to_firebase, 'interval', minutes=5)
    scheduler.start()
    while True:
        time.sleep(1)
