"""
indicators.py — Phiên bản Nâng cấp Follow BTC Trend & High Win Rate
------------------------------------------------------------------
- Lọc tín hiệu Altcoin theo xu hướng chủ đạo của BTC (H4/D1)
- Entry DCA 2 điểm (Entry 1 + Entry 2 DCA) với phân bổ Vol 40% / 60%
- Chốt lời đa tầng: TP1 (R:R 1:1), TP2 (R:R 1:2), TP3 (Kháng cự/Hỗ trợ)
- SL tối ưu theo ATR & Vùng đỉnh/đáy hỗ trợ
"""
...
def generate_signal_for_profile(tf_results, profile_key, btc_context=None, is_btc=False):
    ...
    # 1. Điều kiện kỹ thuật nội tại coin
    ...
    # 2. Bộ lọc TREND BTC (Loại bỏ lệnh Altcoin đi ngược xu hướng BTC)
    if not is_btc and btc_context:
        ...
        if signal_type == "BUY (LONG)" and btc_main_trend != "UP":
            return None  # Bỏ qua nếu BTC không ủng hộ Xu hướng Tăng
        elif signal_type == "SELL (SHORT)" and btc_main_trend != "DOWN":
            return None  # Bỏ qua nếu BTC không ủng hộ Xu hướng Giảm
    ...
    # 3. Tính Entry DCA 2 Điểm & TP/SL
    if "BUY" in signal_type:
        entry_1 = price
        entry_dca = price - (0.75 * atr)  # Vùng chờ mua thêm DCA
...
def calc_position_sizing(entry_1, stop_loss, confluence_pct, profile_key):
    sl_dist_pct = abs(entry_1 - stop_loss) / entry_1 * 100
    risk_pct = 1.5 if confluence_pct >= 80 else 1.0

    leverage_map = {"SCALP": 20, "SWING": 15, "POSITION": 10}
    leverage = leverage_map.get(profile_key, 10)

    notional_pct = (risk_pct / sl_dist_pct) * 100 if sl_dist_pct > 0 else 10.0
    margin_pct = min(round(notional_pct / leverage, 2), 15.0)
